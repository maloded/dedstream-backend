...
export class TelegramService extends Telegraf {
	...
	public async onStart(@Ctx() ctx: any) {
		...
		if (token) {
			...
			if (!authToken.userId) {
				return await ctx.reply('Token does not belong to a valid user');
			}
			...
			await this.connectTelegram(authToken.userId, chatId as string);
	...
	public async onFollows(@Ctx() ctx: Context) {
		...
		if (chatId) {
			...
			if (user && follows.length) {
				...
				const message =
					`<b>🌟 Channels you are subscribed to:</b>\n\n` +
					`${followsList}`;

				await ctx.replyWithHTML(message);
			} else {
				await ctx.replyWithHTML("<b>You don't have followings<b>");
			}
		}
	}
