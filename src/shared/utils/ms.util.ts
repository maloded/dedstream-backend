...
export function ms(str: StringValue): number {
    ...
    const match = /^(?<value>-?(?:\d+)?\.?\d+) *(?<type>milliseconds?|millisecond|millisecs|millisec|ms|seconds?|secs?|sec|s|minutes?|mins?|min|m|hours?|hrs?|hr|h|days?|day|d|weeks?|wks?|wk|w|years?|yrs?|yr|y)?$/i.exec(str);
